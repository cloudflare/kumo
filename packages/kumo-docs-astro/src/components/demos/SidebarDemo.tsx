...
// ---------------------------------------------------------------------------
// 4. Resizable — drag handle with auto-collapse
// ---------------------------------------------------------------------------
  ...
  return (
    <DemoContainer>
      ...
      >
        ...
        <DemoMain>
          <div className="flex flex-col items-center gap-2">
            <kbd className="rounded-lg! ring ring-kumo-line font-sans font-medium text-kumo-default whitespace-nowrap">
              <span className="opacity-50">&#x2318;</span>B
            </kbd>
            <p>Press Cmd+B (Mac) or Ctrl+B to toggle</p>
          </div>
        </DemoMain>
      </Sidebar.Provider>
    </DemoContainer>
...
/** Kitchen sink sidebar showcasing every subcomponent: header with account switcher, search input, groups with labels, collapsible sections with nested expandable, badges, sliding views via Domains, and a footer trigger. */
export function SidebarFullDemo() {
  const [surface, setSurface] = useState<"account" | "domain">("account");

  return (
    <DemoContainer>
      <Sidebar.Provider contained defaultOpen peekable keyboardShortcut="mod+b" className="min-h-0! h-full">
        <Sidebar>
          <Sidebar.Header>
            <AccountSwitcher />
          </Sidebar.Header>
