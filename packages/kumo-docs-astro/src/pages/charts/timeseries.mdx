...
<p>
  Use <code>xAxisTickFormat</code> to customize x-axis labels and
  <code>tooltipTimestampFormat</code> to customize timestamps in standard and
  marker tooltips. Both receive the raw timestamp in milliseconds, so the same
  user-selected time zone can be applied consistently. Tooltip timestamps use
  the browser's locale and time zone when no formatter is provided.
</p>
