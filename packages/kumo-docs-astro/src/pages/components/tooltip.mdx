...
// Then use Tooltip anywhere inside. Replace with localized copy for your app.
const addItemLabel = "Add item";

// Button renders its icon prop as decorative; aria-label names the control.
<Tooltip
  content={addItemLabel}
  render={
    <Button shape="square" icon={PlusIcon} aria-label={addItemLabel} />
  }
/>
```
