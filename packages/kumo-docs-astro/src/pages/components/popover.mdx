...
<p>
  Apply overflow styles to an inner scroll container, not to `Popover.Popup` or
  the deprecated `Popover.Content`. This keeps `Popover.Arrow` outside the
  clipping boundary while the content scrolls independently.
</p>
