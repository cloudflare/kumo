...
<p>
  Base UI exposes the space between the trigger and the viewport edge as
  `--available-height`. This example gives the popup a preferred height of
  320px, then uses the variable as its maximum height so the popup shrinks and
  its inner content scrolls when less space is available.
</p>
