...
<p>
  ...
  transform animation. The example also applies `overflow-auto` directly to
  `Content` to demonstrate that existing scrollable popovers remain compatible.
  New popovers should compose the individual parts as shown in the basic
  example, with scrolling on an inner container when needed.
</p>
