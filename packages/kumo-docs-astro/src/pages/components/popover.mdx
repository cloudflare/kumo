...
<p>
  ...
  `--anchor-height` and `--anchor-width` to match the trigger,
  `--available-height` and `--available-width` to respect the viewport, and
  `--transform-origin` for the popup animation.
</p>
