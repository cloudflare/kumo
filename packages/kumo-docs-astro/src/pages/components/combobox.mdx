...
export default function Example() {
  ...
  return (
    <Combobox
      ...
      onCreate={(nextFruit) => {
        setItems((current) =>
          current.includes(nextFruit) ? current : [...current, nextFruit],
        );
        setValue(nextFruit);
      }}
    >
...
### Creatable
  ...
  <p>
    Pass <code class="text-surface">onCreate</code> to let users add a new option
    when the current input has no exact match. Update your local item list inside
    that callback so the new option persists in the combobox.
  </p>
  <ComponentExample demo="ComboboxCreatableDemo">
    <ComboboxCreatableDemo client:load />
  </ComponentExample>
</ComponentSection>
...
<p>
  By default, <code class="text-surface">Combobox.Content</code> has a max
  height of <code class="text-surface">24rem</code> (384px) or the available
  viewport space, whichever is smaller. The dropdown scrolls automatically when
  content exceeds this height.
</p>
<p>
  To customize the max height, pass a className to{" "}
  <code class="text-surface">Combobox.Content</code>:
</p>
