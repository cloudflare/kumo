...
<p>
  Every combobox trigger or search input needs an accessible name. Prefer the
  root `label` prop for visible field labels. For compact or custom layouts,
  pass `aria-label` or `aria-labelledby` to `Combobox.TriggerInput`,
  `Combobox.TriggerValue`, `Combobox.TriggerMultipleWithInput`, or popup
  `Combobox.Input` as needed. Placeholder text is only a hint and does not
  count as the accessible name.
</p>
...
<p>
  Combobox exposes a few Kumo-owned strings for localizing secondary controls
  and empty states:
</p>

<ul class="list-disc list-inside space-y-1">
  <li>
    <code>Combobox.TriggerInput.clearLabel</code> labels the clear-selection
    button.
  </li>
  <li>
    <code>Combobox.TriggerInput.showOptionsLabel</code> labels the dropdown
    options button.
  </li>
  <li>
    <code>Combobox.Chip.removeLabel</code> labels each chip remove button in
    multiple mode.
  </li>
  <li>
    <code>Combobox.Empty</code> children provide localized empty-state content.
  </li>
</ul>
...
<Combobox multiple items={bots} label="Bots">
  <Combobox.TriggerMultipleWithInput
    placeholder="Select bots"
    renderItem={(bot) => (
      <Combobox.Chip key={bot.value} removeLabel={`Remove ${bot.label}`}>
        {bot.label}
      </Combobox.Chip>
    )}
  />
...
function LanguagePicker() {
  ...
  return (
    <Combobox items={languages} label="Language" filter={filter}>
      {/* ... */}
    </Combobox>
  );
}
...
```tsx
<Combobox items={results} label="Search results" filter={null}>
  ...
</Combobox>
```
