...
import { LinkButtonExternalDemo } from "~/components/demos/LinkButtonDemo";
...
<p>
  Use [`LinkButton`](/components/link-button) when the interaction should
  navigate somewhere but still look like a button. Use `Button` for in-place
  actions like submitting, opening, or toggling UI.
</p>
<ComponentExample
