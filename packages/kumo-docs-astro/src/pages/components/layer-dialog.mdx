...
Consumers cannot mix these layouts or add unrelated footer controls. `Actions` accepts one `Action`, which can optionally expose related menu items. Use the X for read-only content, `Actions` when the user must commit a change, and `LayerDialog.Alert` for destructive or critical confirmations.
...
Pass related actions through `LayerDialog.Action`’s `menu` prop. It renders a standardized split-button caret beside the action. Keep the automatic dismiss button separate. For example, **Save and deploy** can expose **Save as draft** without turning the footer into three peer actions.
...
<p>
  A footer with an automatic dismiss button and exactly one `LayerDialog.Action`.
</p>
...
<p>
  The primary action. Renders a Kumo `Button` and accepts standard button
  attributes such as `onClick`, `disabled`, and `type`. Pass menu items through
  `menu` to render a matching split-button menu.
</p>
