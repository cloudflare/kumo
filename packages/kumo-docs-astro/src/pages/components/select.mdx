...
{/* Select.Value Compound Component */}

<ComponentSection>
  <Heading level={3}>Custom Value Display (Select.Value)</Heading>
  <p>
    For full control over how the selected value is displayed, use the{" "}
    <code class="rounded bg-kumo-control px-1 py-0.5 text-sm">Select.Value</code>{" "}
    compound component. The children function receives the value (or <code class="rounded bg-kumo-control px-1 py-0.5 text-sm">null</code>{" "}
    when nothing is selected), making it clear you need to handle the empty state.
  </p>
...
<Select>
  ...
  <Select.Option value={user1}>{user1.name}</Select.Option>
</Select>
```

  <ComponentExample demo="SelectValueCompoundDemo">
    <SelectValueCompoundDemo client:load />
  </ComponentExample>
</ComponentSection>
