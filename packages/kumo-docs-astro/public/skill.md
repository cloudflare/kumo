...
### 16. Do not use `window.open` if possible
...
```tsx
<Link href="/workers/services">View Workers</Link>
...
### 17. Never use JavaScript for responsive styling
...
```tsx
<LayerCard className="grid gap-4 p-4 md:grid-cols-2">
  <Text>Requests</Text>
  <Text>Errors</Text>
</LayerCard>
```
...
### 18. Never test for the existence of class names
...
### 19. Never mock components in tests
