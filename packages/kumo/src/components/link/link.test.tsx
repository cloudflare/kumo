...
describe("Link", () => {
  ...
  it("exposes a single 'default' variant", () => {
    expect(Object.keys(KUMO_LINK_VARIANTS.variant)).toEqual(["default"]);
  });

  it("applies default variant classes: current color + underline", () => {
    expect(KUMO_LINK_VARIANTS.variant.default.classes).toContain("text-current");
    expect(KUMO_LINK_VARIANTS.variant.default.classes).toContain("underline");
    expect(KUMO_LINK_VARIANTS.variant.default.classes).toContain(
      "link-current",
    );
  });

  it("does not ship a brand-blue link color", () => {
    for (const { classes } of Object.values(KUMO_LINK_VARIANTS.variant)) {
      expect(classes).not.toContain("text-kumo-link");
    }
  });
  ...
  it("should generate variant classes via linkVariants helper", () => {
    expect(linkVariants({ variant: "default" })).toContain("text-current");
    expect(linkVariants({ variant: "default" })).toContain("underline");
  });

  it("should default to the 'default' variant", () => {
    expect(linkVariants()).toBe(linkVariants({ variant: "default" }));
    expect(linkVariants()).toContain("text-current");
