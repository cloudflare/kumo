...
// Extract props from registry. LinkButton has its own registry entry that
// reuses Button's variant table via `KUMO_LINK_BUTTON_VARIANTS`.
const linkButtonProps = registry.components.LinkButton.props;

const variantProp = linkButtonProps.variant as {
  values: string[];
...
const sizeProp = linkButtonProps.size as {
  values: string[];
  classes: Record<string, string>;
  descriptions: Record<string, string>;
  default: string;
