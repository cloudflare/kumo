...
// Extract props from registry. RefreshButton has its own registry entry that
// reuses Button's variant table via `KUMO_REFRESH_BUTTON_VARIANTS`.
const refreshButtonProps = registry.components.RefreshButton.props;

const variantProp = refreshButtonProps.variant as {
  values: string[];
...
const sizeProp = refreshButtonProps.size as {
  values: string[];
  classes: Record<string, string>;
  descriptions: Record<string, string>;
  default: string;
