...
function readItems(
	...
	if (node.type === "Identifier") return readItems(constants.get(node.name), constants);
...
export function assertSelectOutput(
	...
	walk(ast, (node) => {
		if (node.type === "ImportDeclaration" && node.source.value === "@cloudflare/kumo") {
	...
	for (const element of jsxElements) {
		...
		if (name === "select") failures.push("uses native <select> instead of Kumo Select");
		if (disallowedNames.has(name)) failures.push(`uses unsupported ${name} API`);
	}
	...
	if (selectElement) {
		...
		if (!attrs.has("label") && !attrs.has("aria-label") && !attrs.has("aria-labelledby")) {
		...
		if (!attrs.has("onValueChange")) failures.push("missing onValueChange prop");
		if (attrs.has("defaultValue")) failures.push("uses defaultValue instead of controlled empty state");
		...
		for (const label of expectedLabels) {
			if (![...items.values()].includes(label)) failures.push(`items is missing label ${label}`);
		}
	}
